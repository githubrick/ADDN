...
class APNS_Push
{
	...
	function __construct($config)
	{
        ...
        writeToLog($config['server']);
        writeToLog($config['passphrase']);
        writeToLog($config['db']['host']);
        writeToLog($config['db']['dbname']);
        writeToLog($config['db']['username']);
        writeToLog($config['db']['password']);
	...
	function start()
	{
		...
		while (true)
		{
			// Do at most 20 messages at a time. 
