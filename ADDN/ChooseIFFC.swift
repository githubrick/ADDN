...
// This class provides the UI view for users to select the IFFC parameters. 

import UIKit

class ChooseIFFC: UIViewController, UITextFieldDelegate, UIScrollViewDelegate  {
    ...
    override func viewDidLoad() {
        ...
        let IFFC = userDefaults.objectForKey("IFFC") as! NSMutableArray
        textField1.text = IFFC[0] as? String
        textField2.text = IFFC[1] as? String
        numberOfRanges = IFFC.count/2
        var value: String = ""
        //var temp: UITextField = UITextField()
        let height = lessEqualLabel.frame.origin.y
        for (var i = 2; i < IFFC.count; i += 1) {
            value = IFFC[i] as! String
            if(i%2 == 0){
    ...
    override func viewWillDisappear(animated: Bool) {
        ...
        userDefaults.setObject(newdiabetesDuration, forKey: "IFFC")
    ...
    */
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        center.addObserver(self, selector: #selector(ChooseIFFC.keyboardOnScreen(_:)), name: UIKeyboardDidShowNotification, object: nil)
        center.addObserver(self, selector: #selector(ChooseIFFC.keyboardOffScreen(_:)), name: UIKeyboardDidHideNotification, object: nil)
