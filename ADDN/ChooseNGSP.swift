...
// This class provides the UI view for users to select the NGSP parameters. 

import UIKit

class ChooseNGSP: UIViewController, UITextFieldDelegate, UIScrollViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        let NGSP = userDefaults.objectForKey("NGSP") as! NSMutableArray
        textField1.text = NGSP[0] as! String
        textField2.text = NGSP[1] as! String
        numberOfRanges = NGSP.count/2
        var value: String = ""
        //var temp: UITextField = UITextField()
        let height = lessEqualLabel.frame.origin.y
        for (var i = 2; i < NGSP.count; i += 1) {
            value = NGSP[i] as! String
            if(i%2 == 0){
    ...
    override func viewWillDisappear(animated: Bool) {
        ...
        userDefaults.setObject(newdiabetesDuration, forKey: "NGSP")
